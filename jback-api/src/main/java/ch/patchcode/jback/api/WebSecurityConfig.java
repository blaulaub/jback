...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                ...
                .antMatchers(HttpMethod.GET, "/api/v1/persons/for-current-principal").authenticated()
                ...
                .antMatchers(HttpMethod.POST, "/api/v1/clubs/????????-????-????-????-????????????/membership-applications").authenticated()
                // TODO the following access to club membership applications should be restricted
                .antMatchers(HttpMethod.GET, "/api/v1/clubs/????????-????-????-????-????????????/membership-applications").authenticated()
